...
class Reka(object):
    # tu ma byc zapis calej reki z historii rak
    def __init__(self, hand):
        ...
        self._streets()
        self._acts()
        # mam do gotowe do uzycia atrybuty self.strity, self.sb, self.bb,
        # self.gracze, self.actions
    ...
    def _playas(self):
        # dla kazdego gracza obecnego w rece zwraca tuple(int, Gracz), gdzie
        # int to numer jego seata.
        self.players = list()
    ...
    def _button(self):
        # zwraca 4 jezeli tam button byl na Seat 4
    ...
    def _players_positions(self):
        # kazdemu graczowi przyporzadkowuje pozycje
        ...
        if plrs_num == 6:
            ...
            # w hu dealer jest na smallblindzie - pierwszy sie rusza pre i
            # ostatnio post
            positions = ['bu', 'bb']
    ...
    def get_street_action(self, street_name):
        # zwraca slownik z zapisem co kto zrobil na danym stricie, w
        # kolejnosci w jakiej sie to odbywalo
        return self.actions.get(street_name.upper(), {})

    def get_street_bool(self, street_name):
        # zwraca True jezeli byl wylozony dany strit
    ...
    def get_street_players(self, street_name):
        # zwraca jacy gracze byli na danym stricie
...
class Player(object):
    ...
    def rel_pos_post_bool(self, other):
        # zwraca True jezeli gracz ma pozycje postflop nad innym graczem
    ...
    def _players_ai(self, plrs, plrs_ai):
        ...
        if len(plrs) > 1:
            ...
            for plr in plrs:
                ...
                # self.abcd[plr] = self.players[plr]
        for plr in plrs_ai:
            self.players[plr].set_street_ai(self.name)
            # self.abcd[plr] = self.players[plr]
...
class Bet(FacingOpp):
    def __init__(self, reka, strit, player):
        FacingOpp.__init__(self, reka, strit, player)
        self.attrs = {}
        self._better()
        self._raiser()
...
class Db(Bet):
    def __init__(self, reka, strit, gracz, cache=None):
        Bet.__init__(self, reka, strit, gracz)
        self.attrs = {}
        self._initiator()
        self.cache = cache
        self.dbetter = -1
...
class Cb(Db):
    def __init__(self, reka, strit, gracz, cache=None):
        Db.__init__(self, reka, strit, gracz)
        self.attrs = {}
        self.cache = cache
...
class BVsMcb(Cb):
    def __init__(self, reka, strit, gracz, cache=None):
        Cb.__init__(self, reka, strit, gracz)
        self.attrs = {}
        self.cache = cache
...
class Float(BVsMcb):
    # make double float???
    def __init__(self, reka, strit, gracz, cache=None):
        BVsMcb.__init__(self, reka, strit, gracz)
        self.attrs = {}
        self.cache = cache
        self.floater = -1
...
class PlayerGeneral(object):
    ...
    def ai(self):
        ...
        '''
        for name, street in self.hand.streets.items():
            street.players_ai()
        '''
        # if self.hand.get_sd_bool():
        return self.players[self.player].get_street_ai()
        # tmp = self.hand.get_ai_street()
        # if tmp is not None:
        #     tmp_plrs = self.hand.get_players_ai()
        #     if self.player in [str(p) for p in tmp_plrs]:
        #         return tmp
        #     return None
        # return None
...
'''
Dcb moge zrobic z tego czy mogl cbetowac na wczesniejszym stricie i czy betowal
na tym BXB - jak zrobie kolumny z klasy Bet to spoko mozna dostac te state
WSD po R na stricie - musze miec kolumny w ktorych jest info czy gracz raisowal
na danym stricie
'''
